...
function getQuestionMode(bundle) {
...
function getQuestionById(bundle, questionId) {
  return getVisibleQuestions(bundle).find((question) => question.id === questionId) ?? null;
}

function getFirstQuestionId(bundle) {
  return getVisibleQuestions(bundle)[0]?.id ?? "confirm";
}

function getNextQuestionId(bundle, currentQuestionId) {
...
function getPreviousQuestionId(bundle, currentQuestionId) {
  const visible = getVisibleQuestions(bundle);
  const index = visible.findIndex((question) => question.id === currentQuestionId);
  if (index <= 0) {
    return "start";
...
function getCurrentSectionId(bundle) {
...
function getSectionIndex(mode, sectionId) {
...
function createEmptyDraftBundle() {
...
function hydrateDraftBundle(bundle) {
...
function isSeaLike(waterType) {
  return waterType === "sea" || waterType === "brackish";
}

function isFreshwater(waterType) {
  return ["river", "lake", "pond"].includes(waterType);
}

function isBoatLike(tripType) {
  return tripType === "boat";
}

function isShoreLike(tripType) {
  return tripType === "shore";
}

function computeTripValidationState(trip) {
  const errors = [];
  if (!trip.started_at) {
...
function sanitizeTripForSave(trip, status = "final") {
...
function computeCatchCount(trip) {
...
function collectSpeciesLabel(trip) {
  if (trip.result?.result_type !== "caught") {
...
function primarySpeciesName(trip) {
...
function describeLabel(group, value) {
  if (!value) {
    return "未入力";
  }
...
function mapStepToSection(stepId) {
...
function extractCandidatesFromTrip(trip) {
  return {
...
function mergeCandidateData(base = {}, addition = {}) {
  const merged = {};
...
async function saveTripAndDeleteDraft(trip, draftId) {
  if (activeStorageMode === STORAGE_MODE_REMOTE) {
...
async function saveBlob(blobKey, blob) {
...
async function saveBackupStatus(backupStatus) {
...
async function restoreBackupData({ trips, drafts, candidateData }) {
  if (activeStorageMode === STORAGE_MODE_REMOTE) {
...
async function saveRestorePoint(restorePoint) {
  if (activeStorageMode === STORAGE_MODE_REMOTE) {
...
async function restoreFromRestorePoint(restorePointId) {
  if (activeStorageMode === STORAGE_MODE_REMOTE) {
